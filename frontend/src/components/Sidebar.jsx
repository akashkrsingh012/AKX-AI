...
import { Plus, MessageSquare, LogOut, User, PenSquare, Menu, X, CoinsIcon, Image as ImageIcon, FileText } from "lucide-react";
import { useDispatch, useSelector } from "react-redux";
import api from "../utils/axios";
import { setUserData } from "../redux/user.slice";
import { getConversations } from "../features/conversation.api";
import { setConversations, setSelectedConversation, setCurrentView } from "../redux/conversation.slice";
...
const [showBilling, setShowBilling] =useState(false);
  ...
  const handleSelectConversation = async (conversation) => {
    ...
    // Extract artifacts from the last assistant message (messages is an array)
    const lastAssistantMsg = [...messages].reverse().find(m => m.role === "assistant");
    dispatch(setArtifacts(lastAssistantMsg?.artifacts || []));
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2.5 px-4 py-4 border-b border-white/[0.06]">
        ...
        <div className="flex items-center gap-2 flex-1">
          <img src="/logo.png" alt="AKX AI Logo" className="w-6 h-6 object-contain" />
          <span className="text-[16px] font-semibold tracking-tight accent-gradient-text">AKX AI</span>
        </div>
      ...
      <div className="px-4 pt-4 pb-1">
        ...
        <button
          onClick={() => { dispatch(setCurrentView("imageGenerator")); setMobileOpen(false); }}
          className="w-full flex items-center justify-center gap-2 text-sm font-medium text-slate-300 bg-white/[0.05] border border-white/[0.08] rounded-xl py-[10px] mt-2 cursor-pointer hover:bg-white/[0.08] transition-colors duration-150"
        >
          <ImageIcon size={15} />
          Image Generator
        </button>
        <button
          onClick={() => { dispatch(setCurrentView("documentAnalyzer")); setMobileOpen(false); }}
          className="w-full flex items-center justify-center gap-2 text-sm font-medium text-slate-300 bg-white/[0.05] border border-white/[0.08] rounded-xl py-[10px] mt-2 cursor-pointer hover:bg-white/[0.08] transition-colors duration-150"
        >
          <FileText size={15} />
          Document Analyzer
  ...
  return (
    <>
      ...
      <div className={`
        ...
        w-[min(85vw,270px)] h-dvh shrink-0
        bg-app border-r border-app
